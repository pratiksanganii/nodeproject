...
User.prototype.validate = function(){
    return new Promise(async (resolve,reject)=>{
        ...
        if(this.data.password>0 && this.data.password.length<12){this.errors.push("Password must be atleast 12 characters.")}
        if(this.data.password>50){this.errors.push("Password cannot exceed 50 characters.")}
        if(this.data.username>0 && this.data.username<3){this.errors.push("username must be atleast 3 characters.")}
        if(this.data.username>30){this.errors.push("username can not exceed 30 characters.")}
        ...
        if(this.data.username.length>2 && this.data.username.length<31 && validator.isAlphanumeric(this.data.username)){
            ...
            if(userExist){this.errors.push("That username is already taken.")}
        }
...
User.prototype.register = function(){
    return new Promise(async (resolve,reject)=>{
        ...
        // 2. if not validation error save the user data into a database
